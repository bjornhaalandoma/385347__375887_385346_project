...
class TestProject(unittest.TestCase):
    ...
    def test_1_folder_structure(self):
        ...
        self.assertTrue(project_path.exists(),
                        f"No folder found at {project_path}")

        # Main files
        for file in ["main.py", "report.pdf"]:
            with self.subTest(f"Checking file {file}"):
                self.assertTrue((project_path / file).exists(),
        ...
        for file in ["__init__.py", "data.py", "utils.py"]:
            with self.subTest(f"Checking file src/{file}"):
                self.assertTrue((src_path / file).exists(),
                                f"No file {file} found at {src_path}")
        # Methods
        ...
        for file in ["__init__.py", "dummy_methods.py",
            ...
            with self.subTest(f"Checking file methods/{file}"):
                self.assertTrue((method_path / file).exists(),
    ...
    def _import_and_test(self, name, class_name, *args, **kwargs):
        ...
        self.assertIsInstance(
            pred_labels, np.ndarray, f"{name}.{class_name}.fit() should output an array, not {type(pred_labels)}")
        ...
        self.assertIsInstance(
            pred_labels, np.ndarray, f"{name}.{class_name}.predict() should output an array, not {type(pred_labels)}")
        self.assertEqual(pred_labels.shape, training_labels.shape,
    ...
    def test_3a_knn(self):
        ...
        self.assertTrue(np.equal(pred_labels_train, training_labels).all(
        ), f"KNN.fit() is not working on dummy data")
        self.assertTrue(np.equal(pred_labels_test, test_labels).all(),
                        f"KNN.predict() is not working on dummy data")
    ...
    def test_3b_logistic_regression(self):
        ...
        with no_print():
            pred_labels_train = logistic_regression.fit(
    ...
    def test_3c_linear_regression(self):
        ...
        LR_model = self._import_and_test(
            "linear_regression", "LinearRegression", lmda=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-hide', action='store_true',
                        help='Enable printing from the student code')
